...
class GuessStudyEngine implements StudyEngine {
  ...
  static List<GuessUnit> _buildUnits({
    ...
    return items
        .map((item) {
          final List<FlashcardItem> distractorPool = items
              .where((candidate) {
                return candidate.id != item.id;
              })
              .toList(growable: false);
          final List<FlashcardItem> shuffledPool =
              StudyEngineUtils.shuffledCopy(
          ...
          final List<GuessOption> options = <GuessOption>[
            GuessOption(id: item.id.toString(), label: item.backText),
          ];
          for (
            ...
            options.add(
              GuessOption(
                id: distractor.id.toString(),
                label: distractor.backText,
              ),
            );
            index++;
          }
          final List<GuessOption> shuffledOptions =
              StudyEngineUtils.shuffledCopy(values: options, random: random);
          return GuessUnit(
